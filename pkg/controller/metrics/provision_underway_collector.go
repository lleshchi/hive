...
// deprovisioning underway metrics collected through a custom prometheus collector
type deprovisioningUnderwayCollector struct {
	client client.Client

	// minDuration, when non-zero, is the minimum duration afer which clusters deprovisioning
	// will start becoming part of this metric. When set to zero, all clusters deprovisioning
	// will be included in the metric.
	minDuration time.Duration
...
func newDeprovisioningUnderwaySecondsCollector(client client.Client, minimum time.Duration) prometheus.Collector {
...
func (cc clusterSyncFailingCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, cs := range clusterSyncList.Items {
		...
		if cond != nil && cond.Status == corev1.ConditionTrue {
			...
			// check if duration has crosses threshold
			if cc.minDuration.Seconds() <= seconds {
				ch <- prometheus.MustNewConstMetric(
					cc.metricClusterSyncFailingSeconds,
					prometheus.GaugeValue,
					seconds,
					GetNameSpacedName(cs.Namespace, cs.Name),
				)
			}
		}
	}
