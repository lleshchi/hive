...
func TestReconcileClusterPoolNamespace_Reconcile_Movement(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			resources: []runtime.Object{
				testcd.FullBuilder(namespaceName, "test-cd", scheme).
					GenericOptions(testgeneric.WithAnnotation(constants.RemovePoolClusterAnnotation, "true"), testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer")).
					Build(testcd.WithClusterPoolReference("test-pool-namespace", "test-cluster-pool", "test-claim")),
				testcd.FullBuilder(namespaceName, "test-cd-2", scheme).
					GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer")).
					Build(testcd.WithClusterPoolReference("test-pool-namespace", "test-cluster-pool", "test-claim-2")),
		...
		},
		{
			...
			namespaceBuilder:     namespaceBuilder.GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer")),
			expectDeleted:        false,
			validateRequeueAfter: validateNoRequeueAfter,
		},
		{
...
func Test_cleanupPreviouslyClaimedClusterDeployments(t *testing.T) {
	...
	cases := []struct {
		...
		resources: []runtime.Object{
			cdBuilderWithPool("cd1", "test-cluster-pool").GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer")).Build(),
			cdBuilderWithPool("cd2", "test-cluster-pool").GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer")).Build(),
		},
		expectedErr:      "",
		expectedCleanup:  false,
		expectedClusters: 2,
		...
		resources: []runtime.Object{
			cdBuilderWithPool("cd1", "test-cluster-pool").GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer"), testgeneric.WithAnnotation(constants.RemovePoolClusterAnnotation, "true")).Build(),
			cdBuilderWithPool("cd2", "test-cluster-pool").Build(),
		},
		expectedErr:      "",
		expectedCleanup:  false,
		...
		resources: []runtime.Object{
			cdBuilderWithPool("cd1", "test-cluster-pool").GenericOptions(testgeneric.WithAnnotation(constants.RemovePoolClusterAnnotation, "true")).Build(),
			cdBuilderWithPool("cd2", "test-cluster-pool").GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer"), testgeneric.WithAnnotation(constants.RemovePoolClusterAnnotation, "true")).Build(),
		},
		expectedErr:      "",
		expectedCleanup:  true,
		expectedClusters: 1,
		...
		resources: []runtime.Object{
			...
			cdBuilderWithPool("cd4", "test-cluster-pool").GenericOptions(testgeneric.Deleted(), testgeneric.WithFinalizer("test-finalizer"), testgeneric.WithAnnotation(constants.RemovePoolClusterAnnotation, "true")).Build(),
		},
		expectedErr:      "",
		expectedCleanup:  true,
		expectedClusters: 2,
