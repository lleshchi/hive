  ...
  template:
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mapPoolsByType := map[string]string{tc.machinePoolType: pool.Name}
			...
			if tc.expectModified {
				...
				assert.Contains(t, machineSet.ObjectMeta.Labels[constants.MachinePoolNameLabel], pool.Name, "expected label %s to contain %s", constants.MachinePoolNameLabel, pool.Name)
